...
A* 失败会形成下一轮的反馈割。优化器从失败证据中选取可移动的来源、目标和前沿阻挡设备，保存
它们在失败候选中的 $(x_i,y_i,r_i)$，下一精炼轮次：

下一轮优先把这些设备加入部分销毁集，并增加：

$$
\sum_{i\in D_{\mathrm{failed}}}
z_{i,\mathrm{failed}}
\le
|D_{\mathrm{failed}}|-1.
$$

因此至少一台相关设备必须离开失败姿态，之后重新求解并执行完整 A*。

因此整体是有界分解求解：

目标归因与部分销毁 → CP-SAT 设备布局 → 端口漏斗 → 完整 A* 路由。成功候选进入精英档案并按
词典序比较；失败候选生成姿态 no-good 割并反馈下一轮。
...
全局候选被 A* 拒绝时，其失败端点和前沿阻挡设备会生成姿态 no-good 割；若还有后续精炼轮次，
这些设备先进入销毁集，且 CP-SAT 必须改变至少一个失败姿态。
