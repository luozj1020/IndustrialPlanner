 ...
 */
// eslint-disable-next-line @typescript-eslint/no-unused-vars -- Retained as a bounded fallback neighborhood.
...
function hasValidWarehouseHubAdjacency(
  ...
  return hasValidGeneratedWarehouseHubAdjacency({
    devices,
    entityDefinitions: requests.map((request) => request.definition),
  });
}
